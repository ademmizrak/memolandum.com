export class RetroBreakoutEngine {
  ...
  setupLevel() {
    ...
    for(let i=0; i<5; i++) blockData.push({ text: selected[0].english, trText: selected[0].turkish, wordId: 0 });
    for(let i=0; i<5; i++) blockData.push({ text: selected[1].english, trText: selected[1].turkish, wordId: 1 });
    for(let i=0; i<5; i++) blockData.push({ text: selected[2].english, trText: selected[2].turkish, wordId: 2 });
    ...
    for (let i = 0; i < this.blocks.length; i++) {
      ...
      if (b.active) {
        ...
        if (
          ...
          if (this.callbacks.onPlayVoice) this.callbacks.onPlayVoice(b.text, b.isTarget);
