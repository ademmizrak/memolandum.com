...
export default function RetroBreakout({ levelId, langId, onExit, onNextLevel, isAudioEnabled, setIsAudioEnabled, isFxEnabled, setIsFxEnabled }) {
  ...
  const progressTrackerRef = useRef(createSessionProgressTracker('breakout'));
  ...
  const onPlayVoice = useCallback((wordText, isTarget, audioUrl) => {
    // Play SFX if enabled
    if (isFxEnabled) {
       try {
  ...
  useEffect(() => {
    if (activeScreen === 'gameOver' || activeScreen === 'victory') {
      ...
      progressTrackerRef.current.commit({ score: s, gems: 0 });
    }
  }, [activeScreen, uiState.score]);
      ...
      <div className="absolute inset-0 w-full h-full pointer-events-none z-40">
        ...
        {activeScreen === 'pause' && (
          <PauseScreen 
            ...
            onMiniQuizCorrect={() => {
              if (engineRef.current) {
                engineRef.current.lives = Math.min(3, (engineRef.current.lives || 0) + 1);
                onUpdateLives(engineRef.current.lives);
              }
              onScore(parseInt(uiState.score, 10) + 50);
            }}
          />
        )}
