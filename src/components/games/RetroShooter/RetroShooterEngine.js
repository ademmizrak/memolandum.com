...
class Meteor {
  ...
  draw(ctx) {
    ...
    if (this.isBoss) {
      ...
      if (this.word.romanized_script) {
        ctx.font = 'bold 22px Arial, sans-serif';
        ctx.fillText(this.word.english, roundedX + roundedWidth / 2, roundedY + roundedHeight / 2 - 5);
        ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
        ctx.font = 'bold 24px Arial, sans-serif';
        ctx.fillText(this.word.romanized_script, roundedX + roundedWidth / 2, roundedY + roundedHeight / 2 + 18);
      } else {
        ctx.font = 'bold 22px Arial, sans-serif';
        ctx.fillText(this.word.english, roundedX + roundedWidth / 2, roundedY + roundedHeight / 2 + 5);
      }
      ...
      if (this.word.romanized_script) {
        lines = [this.word.english, this.word.romanized_script];
        isRomanized = true;
      } else if (this.word.english.includes('\n')) {
        lines = this.word.english.split('\n');
      } else {
        const textWidth = ctx.measureText(this.word.english).width;
        if (textWidth > roundedWidth - 20 && this.word.english.includes(' ')) {
      ...
      if (lines.length > 1) {
        ...
        ctx.fillStyle = isRomanized ? (isPlayZone ? 'rgba(255, 255, 255, 0.9)' : 'rgba(255, 255, 255, 0.5)') : (isPlayZone ? '#ffffff' : 'rgba(255, 255, 255, 0.4)');
        ctx.font = isRomanized ? (isPlayZone ? 'bold 22px Arial, sans-serif' : '18px Arial, sans-serif') : (isPlayZone ? 'bold 18px Arial, sans-serif' : '16px Arial, sans-serif');
        ctx.fillText(lines[1], roundedX + roundedWidth / 2, roundedY + roundedHeight / 2 + 13);
      } else {
...
export class ShooterGame {
  constructor(vocabulary, jsonFileName, canvasElement, callbacks) {
    ...
    const getVDom = (id) => {
      if (!this.vDom[id]) {
        this.vDom[id] = {
          classList: {
            add: (cls) => { if (cls === 'hidden') this.callbacks.onScreenChange?.(id, true); },
            remove: (cls) => { if (cls === 'hidden') this.callbacks.onScreenChange?.(id, false); }
          },
          style: {},
          textContent: '',
          innerHTML: '',
    ...
    this.phraseDestroyedTimer = 0; // Phrase mode: seconds to wait before spawning next sentence
    ...
    // Cache elements
    this.scoreValEl = this.getElementById('score-val');
    this.highValEl = this.getElementById('high-val');
    this.shieldsEl = this.getElementById('hud-shields');
    this.startScreenEl = this.getElementById('start-screen');
    this.gameOverScreenEl = this.getElementById('game-over-screen');
    this.finalScoreEl = this.getElementById('final-score');
    this.learnedListEl = this.getElementById('learned-list');
  ...
  updateHUD() {
    ...
    const levelValEl = this.getElementById('level-val');
    const masteredValEl = this.getElementById('mastered-val');
    const gemsValEl = this.getElementById('gems-val');

    const levelLabelEl = this.getElementById('level-label');
    const masteredLabelEl = this.getElementById('mastered-label');
    const scoreLabelEl = this.getElementById('score-label');
  ...
  bindEvents() {
    ...
    this.getElementById('start-btn').onclick = () => this.startGame();
    this.getElementById('restart-btn').onclick = () => this.startGame();

    // Bind Pause UI buttons
    this.getElementById('btn-pause').onclick = () => this.togglePause();
    this.getElementById('resume-btn').onclick = () => this.togglePause();
    
    const pauseExamBtn = this.getElementById('pause-exam-btn');
    if (pauseExamBtn) {
      pauseExamBtn.onclick = () => {
        this.state = 'exam';
        this.isPaused = false;
        this.getElementById('pause-screen').classList.add('hidden');
    ...
    const pauseRestartBtn = this.getElementById('pause-restart-btn');
    if (pauseRestartBtn) {
    ...
    this.getElementById('pause-menu-btn').onclick = () => {
      const pauseMenuBtn = this.getElementById('pause-menu-btn');
      if (!this.confirmQuitActive) {
        ...
        const pauseBtn = this.getElementById('btn-pause');
        ...
        this.getElementById('hud').classList.add('hidden');
        this.getElementById('gameCanvas').classList.add('hidden');
        this.getElementById('controls-container').classList.add('hidden');
        if (this.callbacks && this.callbacks.onExit) this.callbacks.onExit();
        updateMainMenuResumeUI();
      }
    };

    // Victory Screen buttons
    const victoryRestartBtn = this.getElementById('victory-restart-btn');
    if (victoryRestartBtn) {
      victoryRestartBtn.onclick = () => {
        this.getElementById('victory-screen').classList.add('hidden');
        this.startGame();
      };
    }

    const victoryNextBtn = this.getElementById('victory-next-btn');
    if (victoryNextBtn) {
    ...
    const victoryMenuBtn = this.getElementById('victory-menu-btn');
    if (victoryMenuBtn) {
  ...
  bindButton(elementId, inputProperty) {
    const btn = this.getElementById(elementId);
  ...
  startGame() {
    ...
    this.phraseDestroyedTimer = 0; // Reset phrase cooldown
    ...
    this.getElementById('hud').classList.remove('hidden');
    
    const victoryScreen = this.getElementById('victory-screen');
    if (victoryScreen) {
  ...
  // Returns true when the active dataset consists of sentences/phrases
  // (i.e. english text contains spaces and is longer than 10 chars)
  get isPhraseMode() {
    if (!this.activeChunk || this.activeChunk.length === 0) return false;
    const sample = this.activeChunk[0].english || '';
    return sample.includes(' ') && sample.length > 10;
  }

  spawnMeteor() {
    if (this.isLevelTransitioning) return;

    const phraseMode = this.isPhraseMode;

    if (phraseMode) {
      // --- PHRASE MODE spawn gates ---
      // Gate 1: If screen is empty after a destroy, wait for sindirim (reading) timer
      if (this.activeMeteors.length === 0 && this.phraseDestroyedTimer > 0) return;
      // Gate 2: Max 2 meteors on screen
      if (this.activeMeteors.length >= 2) return;
      // Gate 3: Second sentence appears earlier to keep pacing faster
      if (this.activeMeteors.length === 1) {
        const existing = this.activeMeteors[0];
        // Only wait until the first sentence has cleared the top area a bit (e.g. 150px)
        if (existing.y < 150) return;
      }
    }
    ...
    // Phrase mode: slower but readable; regular mode: standard speed
    const baseSpeed = phraseMode ? 42 : 55;
    const speedBoost = phraseMode ? Math.min(10, this.score * 0.1) : Math.min(30, this.score * 0.45);
    const speed = (baseSpeed + speedBoost) * this.speedMultiplier;
  ...
  triggerVictory() {
    ...
    // Save learned words to Vocabulary Vault
    if (this.callbacks.onVictory && this.wordsLearnedThisRun?.length > 0) {
      this.callbacks.onVictory(this.wordsLearnedThisRun);
    }
    ...
    const nextBtn = this.getElementById('victory-next-btn');
    if (nextBtn) {
      if (nextSector) {
    ...
    // Display score
    const victoryScoreEl = this.getElementById('victory-score');
    if (victoryScoreEl) {
      victoryScoreEl.textContent = this.score;
    }
    
    // Display mastered words
    const victoryLearnedList = this.getElementById('victory-learned-list');
  ...
  triggerStageCelebration(nextStage) {
    ...
    localStorage.setItem(`memolandum_progress_${nextStage}`, 0);
    localStorage.setItem('memolandum_saved_score', this.score);
    updateMainMenuResumeUI();

    // Show the celebration DOM Overlay
    const celebrationScreen = this.getElementById('celebration-screen');
    const celebrationText = this.getElementById('celebration-text');
    ...
    // Setup Countdown
    let countdown = 4;
    const countdownEl = this.getElementById('celebration-countdown');
    ...
    const skipBtn = this.getElementById('celebration-skip-btn');
    skipBtn.onclick = () => {
      clearInterval(this.celebrationIntervalId);
      this.proceedToNextStage();
    };
  ...
  proceedToNextStage() {
    ...
    // Hide overlay
    this.getElementById('celebration-screen').classList.add('hidden');
    this.isCelebrationActive = false;
    ...
    if (prefetchedData) {
      ...
      const loadingScreen = this.getElementById('loading-screen');
  ...
  update(dt) {
    ...
    // Spawn timer logic
    this.spawnTimer += dt;
    // Phrase mode: poll frequently (gate logic is in spawnMeteor)
    // Regular mode: calm spawn rate scaled by difficulty and speedMultiplier
    const phraseMode = this.isPhraseMode;
    const activeInterval = phraseMode
      ? 0.05  // Check every 50ms — spawnMeteor handles the 30px gate
      : Math.max(2.6, this.spawnInterval - (this.score * 0.005)) / this.speedMultiplier;
    ...
    for (let i = this.activeMeteors.length - 1; i >= 0; i--) {
      ...
      if (meteor.y > 830) {
        ...
        if (meteor.isBoss) {
          ...
          // In phrase mode: short pause so user can mentally reset before next sentence
          if (this.isPhraseMode) this.phraseDestroyedTimer = 0.2;
      ...
      for (let j = this.activeMeteors.length - 1; j >= 0; j--) {
        ...
        if ((meteor.isBoss ? meteor.y > 50 : meteor.y > 240) && this.checkLaserMeteorCollision(laser, meteor)) {
          ...
          if (meteor.hp > 0) {
            ...
            this.screenShake = 3;
            this.hitPauseUntil = performance.now() + 50; 
          } else {
            this.soundManager.playExplosion();
            if (meteor.word && meteor.word.audioUrl) {
              this.soundManager.playWordAudio(meteor.word.audioUrl);
            }
            ...
            // Phrase mode: 0.3s kısa nefes, anlam ekranda görünür, hemen ardından yeni cümle
            if (this.isPhraseMode && !meteor.isBoss) {
              this.phraseDestroyedTimer = 0.3;
            }
    ...
    // Update phrase mode sindirim (reading) cooldown timer
    if (this.phraseDestroyedTimer > 0) {
      this.phraseDestroyedTimer -= dt;
    }
  }

  draw() {
    if (this.state === 'exam') {
      // Clear frame buffer properly
      this.ctx.save();
  ...
  togglePause() {
    ...
    const pauseScreen = this.getElementById('pause-screen');
    const pauseBtn = this.getElementById('btn-pause');
    const pauseExamBtn = this.getElementById('pause-exam-btn');
