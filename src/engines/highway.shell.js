...
class TrafficCar {
  constructor(lane, y, wordText, wordTranslation, isCorrect, isFiller, wordObj) {
    this.lane = lane;
    this.x = lane * 150 + 75;
    this.y = y;
    this.width = 56;
...
export class HighwayGame {
  constructor(vocabulary, jsonFileName, canvasElement, playAudioCallback) {
    ...
    this.vocabulary = vocabulary.map(w => ({
      english: w.original_script || w.english || '',
      turkish: w.meaning || w.turkish || '',
      ...w
    }));
    ...
    this.handleKeyDownBound = (e) => {
      if (this.state === 'playing' || this.state === 'paused') this.handleKeyDown(e);
    };
    this.handleKeyUpBound = (e) => {
      if (this.state === 'playing') this.handleKeyUp(e);
    };
  ...
  startGame() {
    ...
    if (typeof updateMainMenuResumeUI === 'function') updateMainMenuResumeUI();
    ...
    for (let lane = 0; lane < 4; lane++) {
      ...
      if (lane === targetLane) {
        ...
        if (rand < 0.45 && distractorWords.length > 0) {
          ...
          this.trafficCars.push(new TrafficCar(lane, carY, dWord.english, dWord.turkish, false, false, dWord));
        } else if (rand < 0.85 && this.fillers.length > 0) {
          const fWord = this.fillers[Math.floor(Math.random() * this.fillers.length)];
          this.trafficCars.push(new TrafficCar(lane, carY, fWord.english, fWord.turkish, false, true, fWord));
        }
  ...
  triggerLevelComplete() {
    ...
    this.trafficCars = []; // Clear leftover traffic so player doesn't hit them during transition
    this.updateHUD();
  }
  ...
  cleanup() {
    window.removeEventListener('resize', this.resizeHandler);
    if (this.handleKeyDownBound) window.removeEventListener('keydown', this.handleKeyDownBound);
    if (this.handleKeyUpBound) window.removeEventListener('keyup', this.handleKeyUpBound);
    if (window.speechSynthesis) window.speechSynthesis.cancel();
    this.trafficCars = [];
    this.activeParticles = [];
    this.activeFloatingTexts = [];
  ...
  gameLoop(currentTime) {
    ...
    let realDt = (currentTime - this.lastTime) / 16.6667;
    this.lastTime = currentTime;

    if (realDt > 5) realDt = 5;
    if (realDt <= 0 || isNaN(realDt)) realDt = 1;
  ...
  updateGame(dt = 1) {
    ...
    // 2. Collision Detection: Prioritize the CORRECT car!
    let collidedWithCorrect = false;
    for (let i = this.trafficCars.length - 1; i >= 0; i--) {
      const car = this.trafficCars[i];
      if (!car.isCorrect) continue; // Sadece yeşil arabayı kontrol et

      const dx = Math.abs(this.player.x - car.x);
      let dy = Math.abs(this.player.y - car.y);
      const overlapX = 35; 
      const tunnelBonus = Math.max(0, relativeSpeed * 2);
      const overlapY = 65 + tunnelBonus; 

      if (dx < overlapX && dy < overlapY) {
        // Doğru araca çarpmak her zaman doğru kabul edilecek (Turbo patlaması)
        if (this.playAudioCallback) this.playAudioCallback(car.wordObj || car.wordText);
        this.triggerTurboBlast(car);
        collidedWithCorrect = true;
        break;
      }
    }

    // 3. Eğer doğru araca çarpmadıysak, yanlış araçlara çarpmayı kontrol et
    if (!collidedWithCorrect) {
      for (let i = this.trafficCars.length - 1; i >= 0; i--) {
        const car = this.trafficCars[i];
        if (car.isCorrect) continue; // Zaten kontrol edildi
        ...
        if (dx < overlapX && dy < overlapY) {
          if (this.turboActive) {
            // Turbo modundayken yanlış araca çarpmak hasar VERMEZ
            if (this.soundManager) this.soundManager.playDamage();
            for (let j = 0; j < 15; j++) {
            ...
            // Normal hızdayken yanlış araca çarpmak hasar verir!
    ...
    if (!this.isLevelTransitioning && (this.trafficCars.length === 0 || (!correctCarFound && !this.turboActive))) {
      this.trafficCars = [];
      this.spawnTrafficWave();
    }
  ...
  drawBillboardTruck(ctx) {
    ...
    const ty = 180 + bob; // Pushed down from 120 to 180 to stay below the HTML header
    ...
    if (this.activeWord) {
      ...
      const hasRomanized = this.activeWord.romanized && this.activeWord.romanized !== "";
      
      if (hasRomanized) {
        ctx.font = '900 22px "Orbitron", Arial, sans-serif';
        ctx.fillText(this.activeWord.english, tx, ty - 8);
        
        ctx.font = '500 14px "Orbitron", Arial, sans-serif';
        ctx.fillStyle = '#aaaaaa';
        ctx.fillText(`(${this.activeWord.romanized})`, tx, ty + 12);
      } else {
        ctx.font = '900 24px "Orbitron", Arial, sans-serif';
  ...
  drawPlayerCar(ctx) {
    ...
    if (this.activeWord) {
      ...
      if (hasRomanized) {
        ctx.font = 'normal 11px "Orbitron", Arial, sans-serif';
        const rWidth = ctx.measureText(`(${this.activeWord.romanized})`).width;
        if (rWidth > textWidth) textWidth = rWidth;
        ctx.font = 'bold 13px "Orbitron", Arial, sans-serif';
      }
      textWidth += 16;
      
      const boxHeight = hasRomanized ? 36 : 20;
      const boxYOffset = hasRomanized ? 40 : 24;
      
      ctx.beginPath();
      ctx.roundRect(px - textWidth / 2, py - ph / 2 - boxYOffset, textWidth, boxHeight, 5);
      ctx.fill();
      ctx.stroke();
      ...
      if (hasRomanized) {
        ctx.fillText(this.activeWord.english, px, py - ph / 2 - boxYOffset + 10);
        ctx.font = 'normal 11px "Orbitron", Arial, sans-serif';
        ctx.fillStyle = '#aaaaaa';
        ctx.fillText(`(${this.activeWord.romanized})`, px, py - ph / 2 - boxYOffset + 24);
      } else {
        ctx.fillText(this.activeWord.english, px, py - ph / 2 - boxYOffset + 10);
      }
