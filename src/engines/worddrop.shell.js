...
export class WordDropGame {
  ...
  startGame() {
    ...
    const el_start_screen = document.getElementById('start-screen'); if(el_start_screen) el_start_screen.classList.add('hidden');
    const el_game_over_screen = document.getElementById('game-over-screen'); if(el_game_over_screen) el_game_over_screen.classList.add('hidden');
    const el_victory_screen = document.getElementById('victory-screen'); if(el_victory_screen) el_victory_screen.classList.add('hidden');
  }
  ...
  getWordOfLength(L) {
    ...
    if (this.fillers && this.fillers.length > 0) {
      candidates = this.fillers.filter(w => w.english.length < L);
      if (candidates.length > 0) {
        const selected = this.fillers[Math.floor(Math.random() * this.fillers.length)];
        return {
          letters: padWord(selected.english, L),
          english: selected.english.toUpperCase(),
          turkish: selected.turkish.toUpperCase(),
  ...
  togglePause() {
    ...
    if (this.isPaused) {
      ...
      if (pauseBtn) pauseBtn.style.color = 'var(--glow-yellow)';
      if (pauseBtn) pauseBtn.style.borderColor = 'var(--glow-yellow)';
      ...
      if (pauseBtn) pauseBtn.style.color = 'var(--glow-cyan)';
      if (pauseBtn) pauseBtn.style.borderColor = 'var(--border-color)';
    }
  }
  ...
  bindEvents() {
    ...
    if (btnPauseExam) {
      btnPauseExam.onclick = () => {
        ...
        this.isPaused = false;
        const el_pause_screen = document.getElementById('pause-screen'); if(el_pause_screen) el_pause_screen.classList.add('hidden');
  ...
  checkRowClears() {
    ...
    for (let r = 0; r < 19; r++) {
      ...
      if (isFull) {
        ...
        this.checkRowClears(); // Check again since grid shifted down
        return;
      }
    }
  }
  ...
  triggerRowClear(r, englishWord, turkishWord, words = []) {
    ...
    if (englishWord && englishWord.length > 0) {
      ...
      vocabWords.forEach(w => {
        ...
        if (this.playAudioCallback) this.playAudioCallback(w);
  ...
  draw() {
    ...
    // Green flash overlay
    if (this.flashTimer > 0) {
      this.ctx.fillStyle = `rgba(74, 222, 128, ${this.flashTimer * 0.4})`;
      this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);
    }
