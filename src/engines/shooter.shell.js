...
export class ShooterGame {
  constructor(vocabulary, jsonFileName, canvasElement, playAudioCallback) {
    ...
    this.uiLayer.innerHTML = `
      <div id="retro-shooter-ui-layer" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 40;">
        ...
        <div id="hud" style="pointer-events: auto;">
          <div class="hud-item"><span class="hud-label">SHIELDS</span><span id="hud-shields">🛡️ 🛡️ 🛡️</span></div>
          <div class="hud-item"><span id="level-label" class="hud-label">LEVEL</span><span id="level-val" class="hud-value">1</span></div>
          <div class="hud-item"><span id="mastered-label" class="hud-label">MASTERED</span><span id="mastered-val" class="hud-value">0/10</span></div>
          <div class="hud-item"><span class="hud-label">GEMS</span><span id="gems-val" class="hud-value">💎 0</span></div>
          <div class="hud-item"><span id="score-label" class="hud-label">SCORE</span><span id="score-val" class="hud-value">0</span></div>
          <div class="hud-item"><span class="hud-label">HIGH SCORE</span><span id="high-val" class="hud-value">0</span></div>
    ...
    this.flashTimer = 0; // Epoch timestamp (ms) for hit-stop freeze effect
  ...
  gameLoop(currentTime) {
    ...
    // Skip calculations and updates when system is paused or in hit pause
    if (this.isPaused || performance.now() < this.hitPauseUntil) {
      this.draw();
      return;
    }
        ...
        if ((meteor.isBoss ? meteor.y > 50 : meteor.y > 240) && this.checkLaserMeteorCollision(laser, meteor)) {
          ...
          if (meteor.hp > 0) {
            ...
            if (this.playAudioCallback) {
              this.playAudioCallback(meteor.word);
            }
...
function switchMenuTab(tabId) {
  ...
  this.getElementById(tabId).classList.add('active');
...
function setGameSpeed(speed) {
  ...
  const activeBtn = this.getElementById(`speed-${speed}`);
  if (activeBtn) {
...
function showMainMenu() {
  // Unhide menu overlay
  this.getElementById('main-menu').classList.remove('hidden');
...
function switchGameShell(shellName) {
  ...
  const btnShooter = this.getElementById('shell-shooter');
  const btnBreakout = this.getElementById('shell-breakout');
  const btnWordDrop = this.getElementById('shell-worddrop');
  const btnHighway = this.getElementById('shell-highway');
  const btnWordAscent = this.getElementById('shell-wordascent');
  const btnInvaders = this.getElementById('shell-invaders');
...
function initializeGameWithData(data, jsonFileName) {
  ...
  // Hide Menu Overlay
  this.getElementById('main-menu').classList.add('hidden');
...
function restartExam(mode) {
  ...
  this.getElementById('exam-results-screen').classList.add('hidden');
...
window.addEventListener('load', () => {
  ...
  const resumeBtnMain = this.getElementById('resume-btn-main');
  if (resumeBtnMain) {
  ...
  this.getElementById('menu-btn').addEventListener('click', () => {
    if (window.game) {
      cancelAnimationFrame(window.game.loopId);
      window.game.cleanup();
      window.game = null;
    }
    this.getElementById('game-over-screen').classList.add('hidden');
    this.getElementById('hud').classList.add('hidden');
    document.getElementById("gameCanvas").classList.add('hidden');
    document.querySelector('.controls-container').classList.add('hidden');
    showMainMenu();
