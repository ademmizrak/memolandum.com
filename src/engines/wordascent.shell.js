...
export class WordAscentGame {
  ...
  startGame() {
    ...
    const el_gameCanvas = document.getElementById('gameCanvas'); if(el_gameCanvas) el_gameCanvas.classList.remove('hidden');
    const el_hud = document.getElementById('hud'); if(el_hud) el_hud.classList.remove('hidden');
  ...
  bindEvents() {
    this.keydownHandler = (e) => { if (this.state === 'playing') this.handleKeyDown(e); };
  ...
  togglePause() {
    ...
    if (this.state === 'playing') {
      this.state = 'paused';
      this.isPaused = true;
      const el_pause_screen = document.getElementById('pause-screen'); if(el_pause_screen) el_pause_screen.classList.remove('hidden');
    } else {
      this.state = 'playing';
      this.isPaused = false;
      const el_pause_screen = document.getElementById('pause-screen'); if(el_pause_screen) el_pause_screen.classList.add('hidden');
        ...
        if (overlapX && overlapY) {
          ...
          if (p.isGround) {
            ...
            if (landedFloor > this.maxClearedFloor) {
              ...
              if (found) {
                if (this.playAudioCallback) this.playAudioCallback(found);
                if (!this.wordsLearnedThisRun.some(w => w.english === found.english)) {
