...
// Find the constructor of ShooterGame specifically
const targetToReplace = `export class ShooterGame {
  constructor(vocabulary, jsonFileName) {
    this.canvas = document.getElementById('gameCanvas');
    this.ctx = this.canvas.getContext('2d');`;
    
const replacement = `export class ShooterGame {
...
// Replace document.getElementById(...) with this.getElementById(...) inside ShooterGame
// Only AFTER we've injected our wrapper, and we MUST NOT break the wrapper itself!
// Since we used document.createElement in our injected code above, we're safe if we do a global replace
// Wait! We used document.getElementById(id) in the wrapper fallback! 
// Let's replace document.getElementById with this.getElementById everywhere, then restore it in the wrapper!
code = code.replace(/document\.getElementById/g, 'this.getElementById');
code = code.replace(/this\.getElementById\(id\)/g, 'document.getElementById(id)');
code = code.replace(/this\.getElementById\('gameCanvas'\)/g, 'document.getElementById("gameCanvas")');

// Replace document.querySelector('.controls-container')
code = code.replace(/document\.querySelector\\('\\.controls-container'\\)/g, "this.uiLayer.querySelector('.controls-container')");

// Cleanup function patch
...
// Fix global Game assignment
code = code.replace(/window\.Game = Game;/, 'window.Game = ShooterGame;');
