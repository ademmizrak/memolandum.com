// ----------------------------------------------------
// Breakout (DX-Ball) Learning Protocol
// ----------------------------------------------------

// Generic Object Pool Pattern
class BreakoutObjectPool {
...
class Brick {
  ...
  draw(ctx) {
    ...
    ctx.shadowBlur = 6;
    ctx.fillStyle = 'rgba(2, 8, 15, 0.75)';
...
// Floating Text Entity Class (Pedagogical Feedback)
class FloatingText {
  constructor() {
    this.x = 0;
    this.y = 0;
    ...
    this.active = false;
    this.opacity = 1.0;
    this.life = 3.0; // 3 seconds duration
  }
  reset(x, y, text) {
    this.x = x;
    this.y = y;
  ...
  update(dt) {
    if (!this.active) return;
    this.y -= 30 * dt; // float upwards slowly (30px/sec)
    this.life -= dt;
    this.opacity = Math.max(0, this.life / 3.0);
    if (this.life <= 0) {
      this.active = false;
    }
  }
...
// Circle-to-Rectangle Collision Check with Edge Resolution
function checkBallBrickCollision(ball, brick) {
  ...
  if (distanceSquared < ball.radius * ball.radius) {
    ...
    if (closestX === brick.x || closestX === brick.x + brick.width) {
      // Hit left/right vertical edge
      ball.vx = -ball.vx;
      ball.x += (closestX === brick.x) ? -overlapX : overlapX;
    } else {
      // Hit top/bottom horizontal edge
      ball.vy = -ball.vy;
      ball.y += (closestY === brick.y) ? -overlapY : overlapY;
    }
    return true;
  }
  return false;
...
// Main BreakoutGame Class Definition
class BreakoutGame {
  constructor(vocabulary, jsonFileName) {
    ...
    this.ballPool = new BreakoutObjectPool(
      () => new Ball(),
      (ball, x, y, vx, vy) => {
        ball.x = x;
        ball.y = y;
    ...
    this.brickPool = new BreakoutObjectPool(
      () => new Brick(),
      (brick, x, y, width, height, word) => {
        brick.x = x;
        brick.y = y;
        brick.width = width;
        brick.height = height;
    ...
    this.gemPool = new BreakoutObjectPool(
      () => new BreakoutGem(),
      (gem, x, y) => {
        gem.reset(x, y);
      }
    ...
    this.floatingTextPool = new BreakoutObjectPool(
      () => new FloatingText(),
      (ft, x, y, text) => {
        ft.reset(x, y, text);
      }
    ...
    this.particlePool = new BreakoutObjectPool(
      () => new Particle(),
  ...
  update(dt) {
    ...
    for (let i = this.activeBalls.length - 1; i >= 0; i--) {
      ...
      if (ball.vy > 0 &&
          ...
          ball.x + ball.radius >= this.paddle.x - this.paddle.width / 2 && ball.x - ball.radius <= this.paddle.x + this.paddle.width / 2) {
    ...
    for (let i = this.activeGems.length - 1; i >= 0; i--) {
      ...
      if (gem.active &&
          gem.y >= this.paddle.y && gem.y <= this.paddle.y + this.paddle.height &&
          gem.x >= this.paddle.x - this.paddle.width / 2 && gem.x <= this.paddle.x + this.paddle.width / 2) {
  ...
  updateExam(dt) {
    ...
    for (let i = this.activeBalls.length - 1; i >= 0; i--) {
      ...
      if (target && target.active && !window.examEngine.inTransition) {
        if (ball.y - ball.radius <= target.y + target.height && ball.y + ball.radius >= target.y &&
          ...
          if (brick.hp <= 0) {
            ...
            if (isCorrect) {
              ...
              if (!window.examEngine.currentQuestionMissed) {
                ...
                window.examEngine.results.push({ word: window.examEngine.currentWord, hit: true });
              ...
              if (!window.examEngine.currentQuestionMissed) {
                ...
                window.examEngine.results.push({ word: window.examEngine.currentWord, hit: false });
